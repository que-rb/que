...
KILLER_TESTS = [
  ...
  "Que::Job::.bulk_enqueue#test_0005_should error appropriately on an anonymous job subclass",
  ...
  "Que::JobBuffer::available_priorities::when the maximum buffer size is zero::and all the workers are free#test_0001_should not include any buffer space in the available counts",
  ...
  "Que::Job::run_synchronously=::on the base class#test_0002_should ignore jobs that are scheduled for a future date",
  "Que::Job::run_synchronously=::on the base class#test_0001_should cause jobs to be worked synchronously",
  ...
  "Que::Migrations::que_state trigger::the notification metadata::when the job is wrapped by ActiveJob#test_0001_should report the wrapped job class",
  ...
  "running jobs via ActiveJob::when running synchronously#test_0001_shouldn't fail if there's no DB connection",
  "running jobs via ActiveJob::when running synchronously#test_0002_should propagate errors raised during the job",
  "Que::Job::the JobClass.enqueue() method when run_synchronously is set::#handle_error#test_0001_should be passed the error object and expose the correct error_count",
  "Que::Job::the JobClass.enqueue() method when run_synchronously is set::#handle_error#test_0002_should make it easy to signal that the error should be notified",
  "Que::Job::the JobClass.enqueue() method when run_synchronously is set::#handle_error#test_0005_should support the use of GlobalId arguments",
  "Que::Job::the JobClass.enqueue() method when run_synchronously is set::#handle_error#test_0003_should make it easy to signal that the error should not be notified",
  "Que::Job::the JobClass.enqueue() method when run_synchronously is set::#handle_error#test_0004_when it raises an error of its own should notify it as well",
  ...
  "Que::JobBuffer::available_priorities::when the maximum buffer size is zero::and the low-priority workers are busy#test_0001_should only ask for jobs at the next priority level",
  "Que::ConnectionPool::for a sequel pool::.execute#test_0002_should reuse the same connection when inside a checkout block",
  "Que::ConnectionPool::for a sequel pool::.execute#test_0001_should delegate to the connection",
  "Que::JobBuffer::during instantiation#test_0001_should raise an error if passed a maximum buffer size less than zero",
  "Que::Sequel::Model::finished#test_0001_should return a dataset of jobs that have finished",
  "Que::ConnectionPool::for a active_record pool::.checkout#test_0002_should be reentrant",
  "Que::ConnectionPool::for a active_record pool::.checkout#test_0001_should yield a Connection object",
